...
class formulir_1111_b3(osv.osv):
    ...
    def function_amount_all(self, cr, uid, ids, name, args, context=None):
        ...
        obj_pajak_formulir_1111_b3_detail = self.pool.get('pajak.detail_formulir_1111_b3')
        
        for formulir in self.browse(cr, uid, ids):
            kriteria = [('formulir_id', '=', formulir.id)]
            detail_ids = obj_pajak_formulir_1111_b3_detail.search(cr, uid, kriteria)
            if detail_ids:
                for detail in obj_pajak_formulir_1111_b3_detail.browse(cr, uid, detail_ids):
    ...
    _columns =  {
                                ...
                                'company_id' : fields.many2one(string='Perusahaan', obj='res.company', required=True, readonly=True, states={'draft':[('readonly',False)]}), #TODO: Ticet #73
                                'nama_pkp' : fields.char(string='Nama PKP', size=255, required=True, readonly=True, states={'draft':[('readonly',False)]}), #TODO: Ticket #73
                                'npwp' : fields.char(string='NPWP', size=50, required=True, readonly=True, states={'draft':[('readonly',False)]}), #TODO: Ticket #73
                                'masa_pajak_id' : fields.many2one(string='Masa Pajak', obj='pajak.masa_pajak', required=True, readonly=True, states={'draft':[('readonly',False)]}), #TODO: Ticket #73
                                'pembetulan_ke' : fields.integer(string='Pembetulan Ke-', required=True, readonly=True, states={'draft':[('readonly',False)]}), #TODO: Ticket #73
                                'detail_ids' : fields.one2many(string='Detail', obj='pajak.detail_formulir_1111_b3', fields_id='formulir_id', readonly=True, states={'draft':[('readonly',False)]}), #TODO: Ticket #73
    ...
    def delete_workflow_instance(self, cr, uid, id):
        ...
        wkf_service = netsvc.LocalService('workflow')
        wkf_service.trg_delete(uid, 'pajak.formulir_1111_b3', id, cr)
    ...
    def create_workflow_instance(self, cr, uid, id):
        ...
        wkf_service = netsvc.LocalService('workflow')
        wkf_service.trg_create(uid, 'pajak.formulir_1111_b3', id, cr)
    ...
    def create_sequence(self, cr, uid, id):
        ...
        if formulir.name == '/':
            ...
            if formulir.company_id.sequence_formulir_1111_b3.id:
                sequence = obj_sequence.next_by_id(cr, uid, formulir.company_id.sequence_formulir_1111_b3.id)
...
class detail_formulir_1111_b3(osv.osv):
    _name = 'pajak.detail_formulir_1111_b3'
    _description = 'Formulir 1111 B3'
    _columns =  {
                                ...
                                'formulir_id' : fields.many2one(string='Formulir 1111 B3', obj='pajak.formulir_1111_b3'),
                                }
detail_formulir_1111_b3()
